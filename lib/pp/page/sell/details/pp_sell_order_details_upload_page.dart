...
/// 订单详情-购买-上传存款凭证
class PpSellOrderDetailsUploadPage extends StatefulWidget {
  String order_id;

  PpSellOrderDetailsUploadPage({super.key, required this.order_id});

  @override
  _PageState createState() => _PageState(order_id: order_id);
}

class _PageState extends State<PpSellOrderDetailsUploadPage> {
  String order_id;
  ...
  Future<void> _submit_credential() async {
    ...
    list.add(_saveUrl1);
    list.add(_saveUrl2);
    PPHttpClient().post(PpUrlConfig.submit_credential, data: {
      'order_id': order_id,
      'payment_proof': getTaskScreen(list),
    }, onSuccess: (jsonMap) {
      Toast.show('上传成功');
      Navigator.pop(context);
      goPpBuyOrderDetailsWaitingReleasePage(context, order_id: order_id);
    }, onError: (code, msg) {
      Toast.show(msg);
    });
  }
